...
class DataLoader():
	...
	def get_test_data(self, seq_len, ndim, normalise):
		'''
	...
	def _next_window(self, i, seq_len, ndim, normalise):
		'''Generates the next data window from the given index location i'''
		window = self.data_train[i:i+seq_len]
		window = self.normalise_windows(window, ndim, single_window=True)[0] if normalise else window
		x = window[:-1]
		if ndim == 1:
			# one dim
			y = window[-1]
